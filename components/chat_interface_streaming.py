...
class ChatInterfaceStreaming:
    ...
    def run(self):
        ...
        if prompt := st.chat_input("What's up?"):
            ...
            print(f"Response_stream: {response['stream']}")
        ...
        for event in response_stream:
            if "contentBlockDelta" in event:
                ...
                if "toolUse" in delta:
                    ...
                    print(f"Accumulating toolUse input: {tool_use_input}")
    ...
    def execute_tool(self):
        ...
        tool_response = self.run_tool(tool_name, tool_args)["response_text"]
        try:
            self.response_df = self.run_tool(tool_name, tool_args)["response_df"]
        except:
